...
namespace MyExperiment.SEProject
{
    ...
    public class NFrame
    {
        ...
        public NFrame(SKBitmap bmp, string videoName, string label, int index, int frameWidth, int frameHeight, ColorMode colorMode)
        {
            this.index = index;
            this.label = label;
        ...
        {
            SKBitmap img = ResizeSKBitmap (image, frameWidth, frameHeight);
            ...
            for (int heightCount = 0; heightCount < img.Height; heightCount++)
            {
                ...
                {
                    SKColor pixel = img.GetPixel(widthCount, heightCount);
                    switch (colorMode)
                    {
                        ...
                        case ColorMode.BLACKWHITE:
                            ...
                            double luminance = (3 * pixel.Red + pixel.Blue + 4 * pixel.Green) >> 3;
                        ...
                        case ColorMode.BINARIZEDRGB:
                            ...
                            imageBinary.AddRange(new List<int>() { (pixel.Red > 255 / 2) ? 1 : 0, (pixel.Green > 255 / 2) ? 1 : 0, (pixel.Blue > 255 / 2) ? 1 : 0 });
                            break;
                        case ColorMode.PURE:
                            imageBinary.AddRange(ColorChannelToBinList(pixel.Red));
                            imageBinary.AddRange(ColorChannelToBinList(pixel.Green));
                            imageBinary.AddRange(ColorChannelToBinList(pixel.Blue));
                            break;
                    }
                }
            }
        ...
        {
                SKBitmap temp = IntArrayToSKBitmap (EncodedBitArray);
        ...
        /// <returns></returns>
        public static SKBitmap ResizeSKBitmap(SKBitmap bmp, int width, int height)
        {
            SKBitmap result = new SKBitmap(width, height);
            bmp.ScalePixels(result, SKFilterQuality.High);
            return result;
        }
        /// <summary>
        ...
        public SKBitmap IntArrayToSKBitmap(int[] EncodedBitArray)
        {
            int[] rgb = { 0, 0, 0 };
            SKBitmap output = new SKBitmap(frameWidth, frameHeight);
                ...
                for (int x = 0; x < frameWidth; x += 1)
                {
                    switch (colorMode)
                    {
                        case ColorMode.BLACKWHITE:
                            ...
                            {
                                output.SetPixel(x, y, new SKColor(255, 255, 255));
                            }
                            else
                            {
                                output.SetPixel(x, y, new SKColor(255, 0, 0));
                            }
                            break;
                        case ColorMode.BINARIZEDRGB:
                            for (int i = 0; i < 3; i += 1)
                            ...
                            output.SetPixel(x, y, new SKColor((byte)rgb[0], (byte)rgb[1], (byte)rgb[2], 255));
                            break;
                        case ColorMode.PURE:
                            for (int i = 0; i < 3; i += 1)
                            {
